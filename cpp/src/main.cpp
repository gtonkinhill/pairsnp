...
int main(int argc, char *argv[])
{
  ...
  int opt, quiet=0, csv=0, corner=1, allchars=0, keepcase=0, dist=1;
  while ((opt = getopt(argc, argv, "hqscv")) != -1) {
    switch (opt) {
  ...
  if(dist){
    sp_umat total_snps = sum(sparse_matrix_A + sparse_matrix_C + sparse_matrix_G + sparse_matrix_T, 1);
    ...
    sp_umat binary_snps = spones(sparse_matrix_A + sparse_matrix_C + sparse_matrix_G + sparse_matrix_T);
